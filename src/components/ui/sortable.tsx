...
function SortableContent(props: SortableContentProps) {
  const { strategy: strategyProp, render: renderProp, withoutSlot, ref, ...contentProps } = props

  const context = useSortableContext(CONTENT_NAME)

  const element = useRender({
    defaultTagName: "div",
    props: { "data-slot": "sortable-content", ...contentProps, ref },
    render: renderProp,
    state: {}
  })
  ...
  return (
    <SortableContentContext.Provider value={true}>
      <SortableContext items={context.items} strategy={strategyProp ?? context.strategy}>
        {withoutSlot ? contentProps.children : element}
      </SortableContext>
...
function SortableItem(props: SortableItemProps) {
  const { value, style, asHandle, render: renderProp, disabled, className, ref, ...itemProps } = props
...
function SortableItemHandle(props: SortableItemHandleProps) {
  const { render: renderProp, disabled, className, ref, ...itemHandleProps } = props
